...
import { handleSlashCommand } from "../src/web/slash-commands.js";
...
describe("slash commands", () => {
	it("handles help clear status tools memory trace and exit", async () => {
		const state = new ChatState({ agentName: "Agent", agentId: "agent", model: "model", provider: "provider", toolProfile: "coding", mcpServerCount: 2, cwd: ".", sessionId: "session" });
	...
	it("summarizes and searches memory", async () => {
		const state = new ChatState({ agentName: "Agent", agentId: "agent", model: "model", provider: "provider", toolProfile: "coding", cwd: ".", sessionId: "session" });
		const chat = fakeChat({
