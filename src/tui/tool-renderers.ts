...
import { renderBashResult } from "./bash-renderer.js";
...
export function renderToolResult(result: ToolResult, width: number): string[] {
	if (result.call.name === "bash") {
		const bashLines = renderBashResult(result, width);
		if (bashLines.length > 0) return withStatus(result, bashLines, width);
	}
...
function withStatus(result: ToolResult, lines: string[], width: number): string[] {
	return [truncate(`${statusPrefix(result)} ${result.call.name}`, width), ...lines];
}
